...
private:
  unsigned int size_N;
  vector<vector <int>> SeqListArray;
};
...
void SeqList::Action(int action, int x, int y)
{
  ...
  switch(action) {
    ...
    case 2:{
            //Action 2
              int seq_size = SeqListArray[seq].size();
              assert(seq_size>=0);
              if (debug_mode)
                DEBUG(seq_size);

              int location = y % seq_size;
              int result = SeqListArray[seq][location];
              //
              if (debug_mode)
...
int SeqList::PrintLists()
{
  ...
  for (int i=0; i<n; ++i)
  {
    ...
    int m = SeqListArray[i].size();
    for (int j=0;j<m; ++j)
      cout << SeqListArray[i][j] <<" ";
    cout << ";"<<endl;
  }
  return 0;
}
...
int SeqList::ElementRemove(int position)
{
  ...
  int last_element = SeqListArray[position].back();
  SeqListArray[position].pop_back();
...
int SeqList::ElementAppend(int position, int value)
{
  ...
  SeqListArray[position].push_back(value);
  return 0;
}
